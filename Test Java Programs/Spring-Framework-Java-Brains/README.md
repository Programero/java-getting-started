...
Maven follows a **build lifecycle** with predefined phases. When you run a phase, Maven executes all preceding phases automatically:

#### `clean` - Clean Build Artifacts
- **Purpose**: Removes all files generated by previous builds
- **What it does**: Deletes `target/` directory and all build artifacts
- **Use case**: Start fresh build, resolve build issues
```bash
mvn clean
```
...
- **What it does**: Compiles `.java` files in `src/main/java/` to `target/classes/`
- **Bound goals**: `compiler:compile`
- **Use case**: Just compile without testing
```bash
mvn compile
```

#### `test` - Run Unit Tests
- **Purpose**: Runs unit tests to verify code quality
- **What it does**: Compiles test sources, runs JUnit/TestNG tests
- **Bound goals**: `compiler:testCompile`, `surefire:test`
- **Use case**: Verify code works before packaging
```bash
mvn test
```

#### `package` - Create Package
- **Purpose**: Packages compiled code into distributable format
- **What it does**: Creates JAR/WAR/EAR files in `target/` directory
- **Bound goals**: `jar:jar` (for JAR projects)
- **Use case**: Create deployable artifacts
```bash
mvn package
```

#### `install` - Install to Local Repository
- **Purpose**: Installs package to local Maven repository
- **What it does**: Copies artifact to `~/.m2/repository/` for reuse
- **Bound goals**: `install:install`
- **Use case**: Make project available as dependency for other local projects
```bash
mvn install
```

#### `deploy` - Deploy to Remote Repository
- **Purpose**: Deploys package to remote repository for sharing
- **What it does**: Uploads artifact to remote repository (Nexus, Artifactory)
- **Bound goals**: `deploy:deploy`
- **Requires**: Repository credentials and configuration
- **Use case**: Share artifacts with team/organization
```bash
mvn deploy
```
...
```
mvn deploy → clean + compile + test + package + install + deploy
mvn install → clean + compile + test + package + install
mvn package → clean + compile + test + package
```


> **Note**: In this project we only need `clean`, `compile`, and `package` commands.
...
We created a **deployment folder** with a single Fat JAR file containing all dependencies, which is a copy of target/Spring-Framework-Java-Brains-1.0-SNAPSHOT.jar for the following reasons:
...
### Build Artifacts - Two JAR Files

`mvn package` creates two JAR files when using the Maven Shade plugin. Here's why:

```
target/
├── original-Spring-Framework-Java-Brains-1.0-SNAPSHOT.jar  # 9.5KB (small)
└── Spring-Framework-Java-Brains-1.0-SNAPSHOT.jar          # 7.3MB (large)
```

#### What Each JAR Contains

**1. `original-*.jar` - The Original JAR**
- Created by the standard `maven-jar-plugin`
- Contains **only your compiled classes** + resources
- **No dependencies included**
- Size: ~9.5KB (just your code)

**2. `Spring-*.jar` - The Shaded/Fat JAR**
- Created by the `maven-shade-plugin`
- Contains **your code + ALL dependencies merged together**
- **Fully self-contained executable JAR**
- Size: ~7.3MB (includes Spring Framework, GSON, etc.)
